  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = async (event) => {
      ...
      try {
        ...
        alert("Gemini failed to parse. Try using clear text or supported documents.");
      } finally {
  ...
  return (
    <div className="space-y-6 pb-40 animate-fadeIn">
      ...
      <div className="flex items-center justify-between px-1">
        <div>
          <h2 className="text-2xl font-black tracking-tighter text-slate-900 dark:text-white">Financial Assets</h2>
          <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Global Transaction Feed</p>
        </div>
      ...
      <div className="fixed bottom-28 left-1/2 -translate-x-1/2 z-40 no-print flex gap-2">
        <button 
          onClick={() => fileInputRef.current?.click()}
          className="flex items-center gap-2 bg-slate-900 dark:bg-slate-100 text-white dark:text-slate-900 px-6 py-3 rounded-full shadow-2xl font-black text-[10px] uppercase tracking-widest active:scale-95 transition-all"
        >
          <span className="material-symbols-outlined text-sm">upload_file</span>
          <span>Import Docs (PDF, CSV, XLS)</span>
        </button>
        <input 
          type="file" 
          ref={fileInputRef} 
